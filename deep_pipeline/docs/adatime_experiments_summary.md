...
Non-medical benchmark demonstrating generality of frozen-model retrieval translation beyond EHR data.
The retrieval translator is adapted to work with AdaTime's 1D-CNN backbone: a CNN is frozen on the SOURCE domain, and the translator maps TARGET data to look source-like so the frozen CNN can classify it.
...
- **Translator training**: 30-100 epochs, patience=10, `val_fraction=0.1`, best-epoch selection via val loss
- **Translator HP**: `lambda_fidelity=0.01`, `k_neighbors=8`, `lr=5e-4`
...
- **Translator training**: 100 epochs, `val_fraction=0.0`, `patience=0` (no early stopping), last-epoch model
- **Translator HP**: `lambda_fidelity=0.01`, `k_neighbors=8`, `lr=5e-4` (default)
- Close to AdaTime protocol but translator trains for 100 epochs instead of 40
- Used for: `HHAR_adatime_protocol`, `WISDM_adatime_protocol`, `HAR_adatime_protocol` (a6000)

### 3. "adatime_ep40" (Correct AdaTime Protocol)
- **CNN training**: 40 epochs, no early stopping, `val_fraction=0.0`, last-epoch model
- **Translator training**: 40 epochs, `val_fraction=0.0`, `patience=0` (no early stopping), last-epoch model
- **Translator HP**: `lambda_fidelity=0.01`, `k_neighbors=8`, `lr=5e-4` (default)
...
- **Translator training**: 40 epochs, `val_fraction=0.0`, `patience=0` (no early stopping), last-epoch model, **Adam(β=0.5, 0.99), wd=1e-4**
- **This is the fully correct AdaTime protocol** -- both CNN and translator use the exact optimizer settings from AdaTime Section 3.7
...
- Also used for v3 HP sweeps: `HAR_v3_*` (same protocol, different translator HP)

### 5. "v4" Protocol (Epoch-Budget-Corrected, Fully Compliant)
- **CNN training**: 40 epochs, no early stopping, `val_fraction=0.0`, last-epoch model, **Adam(β=0.5, 0.99), wd=1e-4, no LR scheduler**
- **Translator training**: **30 epochs** (+ 10 pretrain = **40 total**), `val_fraction=0.0`, last-epoch, **Adam(β=0.5, 0.99), wd=1e-4**
...
- **Translator training**: **40 task epochs, 0 pretrain** (= **40 total**), `val_fraction=0.0`, last-epoch, **Adam(β=0.5, 0.99), wd=1e-4**
...
**Note**: HAR source-only MF1 is inflated by PyTorch 2.4 vs AdaTime's PyTorch 1.7 (83.0-85.0 vs published 65.9). HAR translator results beat AdaTime published E2E baselines, but the source-only baseline comparison is confounded.

All HAR experiments run on **a6000** except `HAR_adatime_knb16` (local).

| Variant | Protocol | Epochs | Fidelity | k_neighbors | Src MF1 | Trans MF1 | Delta | W/L |
|---|---|---|---|---|---|---|---|---|
...
**Note on v2 source-only drop**: v2 CNN uses Adam(β=0.5, 0.99) per AdaTime protocol. This produces a weaker CNN (80.0 vs 83.3 with default betas), reducing the translator's ceiling. Despite the weaker CNN, translator still reaches 90.9 MF1.
...
| Scenario | Src MF1 | Trans MF1 | Delta |
|---|---|---|---|
...
**Note on v2 source-only**: v2 CNN with Adam(β=0.5, 0.99) gives src=56.5 (vs 59.6 with default betas). Translator delta (+31.1) is slightly better than pre-v2 ep40 delta (+29.2 at fid=0.01) despite weaker CNN, showing the protocol-compliant optimizer is actually better for translation.
...
Long-sequence (3000 timesteps), single-channel EEG. Requires chunking for translator (CNN handles full-length natively). This dataset tested the limits of our approach on low-channel, long-sequence data.

#### SSC Experiment Variants

| Variant | Location | Chunk Size | d_latent | d_model | Fidelity | Protocol | Src MF1 | Trans MF1 | Delta | W/L |
|---|---|---|---|---|---|---|---|---|---|---|
...
**v2 SSC result**: All v2 configs HURT performance. Adam(β=0.5/0.99) CNN produces different features that the translator cannot improve.
...
| Chunk Size | Src MF1 | Trans MF1 | Delta | W/L | Notes |
|---|---|---|---|---|---|
...
| Variant | Location | Chunk Size | Src MF1 | Trans MF1 | Delta | W/L |
|---|---|---|---|---|---|---|
...
**v2 MFD result**: v2 Adam(β=0.5/0.99) CNN is much stronger (83.7 vs 77.8 src MF1), leaving almost no room for adaptation (+0.3). Pre-v2 CNN had more domain shift and translator helped more (+6.1).
...
Both CNN and translator use Adam(β=0.5, 0.99), wd=1e-4, 40 epochs, last-epoch, no val split, no LR scheduler.

| Dataset | Best v2 Config | Src MF1 | Trans MF1 | Delta |
|---|---|---|---|---|
...
| Dataset | Best ep40 Config | Src MF1 | Trans MF1 | Delta | W/L |
|---|---|---|---|---|---|
...
| Config | d_model | lr | fid | k | Trans MF1 | Delta |
|---|---|---|---|---|---|---|
...
| Config | output_mode | pretrain | d_model | lr | fid | Trans MF1 |
|---|---|---|---|---|---|---|
...
| Dataset | Variant | lr | n_cross | k | fid | Trans MF1 |
|---|---|---|---|---|---|---|
...
| Dataset | Trans MF1 | 95% CI | Δ vs Source-Only | p-value | vs Best E2E |
|---|---|---|---|---|---|
...
| Dataset | Seeds | Trans MF1 | 95% CI | Δ (pooled) | p-value |
|---|---|---|---|---|---|
...
| Dataset | Src MF1 | Trans MF1 | Delta |
|---|---|---|---|
| HAR | 83.0 | 90.4 | +7.4 |
| HHAR | 61.2 | 83.2 | +22.0 |
...
- **Full-length**: train CNN on full sequences, translate in chunks (128/256/512 steps)

The downsampled approach inflates translator gains because avg-pooling destroys the signal structure, creating a weaker CNN baseline. The translator partially recovers this signal loss, but this is an artifact of a bad baseline. Full-length source-only matches AdaTime published numbers (SSC 51.9 vs 51.7), confirming full-length is the correct protocol.

---

## Protocol Evolution Notes

1. **Direction correction**: Initial implementation trained LSTM on TARGET domain and translated SOURCE -> target-like. AdaTime trains CNN on SOURCE domain and translates TARGET -> source-like. Fixed early in the campaign.
...
1. **v4/v5 protocol (fully AdaTime-compliant) beats all published E2E DA on ALL 5/5 datasets** with a strictly frozen backbone. 3-dataset mean 84.2 vs CoTMix 79.0 (+5.2). 5-dataset mean 82.9 vs DIRT-T 77.3 (+5.6). Multi-seed: HAR 94.1±0.0 (perfectly stable), HHAR 87.0±0.7, WISDM 70.3±1.5, SSC 66.2±0.2. All seeds beat published baselines on all datasets.
2. **Adam(β=0.5, 0.99) optimizer actually helps translation**: Despite producing weaker CNNs (HAR src dropped 83.3→80.0), the translator improved significantly on HHAR (+4.4) and WISDM (+9.8). The aggressive momentum may help the translator escape poor local minima.
...
6. **HAR confounded**: PyTorch 2.4 inflates source-only by ~14pp vs published. Translator still beats published DANN (93.0 vs 88.3).
...
2. **Implicit regularization**: The skip connection constrains the translator to learn small corrections. Even with low lambda_fidelity (0.01), the output stays structurally similar to the input. Absolute mode with low fidelity has no such floor and can collapse (decoder produces mode-specific fixed patterns regardless of input).
