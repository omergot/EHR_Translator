...
EHR sits between AdaTime's two clusters: it satisfies manifold coherence (the autoencoder pretrain on MIMIC produces a coherent target manifold; the shared encoder already latent-matches) but fails gradient-headroom (`gradient_bottleneck_analysis.md` shows fidelity gradient is 3–10× the task gradient on EHR, so cross-attention is bandwidth-starved).
...
### 5.4 Operational discriminator

A 1-epoch measurement on a quantity the codebase already logs:

- **If** `cos(task, fidelity) ≥ +0.5` AND `fid/task ratio < 1` AND `channel count < 5` → default `pretrain_epochs = 0`.
- **Otherwise** → default `pretrain_epochs ≥ 10`.

The cosine threshold is provisional — calibrated on Mortality (cos = +0.84 → drop fidelity) and Sepsis (cos = −0.21 → keep fidelity) at multi-seed; the AdaTime side is the regime where the rule trivially predicts `p = 0` because the cosine is high and the channel count is low. A direct cos measurement on AdaTime is `adatime_pretrain_ablation.md` §6 L121's open follow-up.
...
| Rule | AdaTime tier-stable? (Full ↔ Tiny at n=3) | EHR cohort-stable? (eICU ↔ HiRID at n=3) | Cross-benchmark? |
|---|---|---|---|
| Tiny ≥ Full | **Yes 5/5** at measured tiers | not directly tested (Goal-A/B is different design) | AdaTime tier-stable; EHR transport open (§9 #2) |
| `pretrain_epochs = 0` universal | **Yes 5/5** (HHAR/WISDM at z > 2.95σ 5-seed reverse) | catastrophic 5/5 (`p=0` hurts on every task) | **Cross-benchmark split** (clean) |
| `output_mode` regime-split (residual when `p=0`; else absolute, provided `λ_recon > 0`) | **Yes 5/5** AdaTime (toggled ablation; HHAR-`p10`-absolute strict-toggle in the absolute branch, +3.6 MF1) | **Yes 5/5** EHR (R7 forces `p > 0`; clean C8 strict toggle, n=3) | **Cross-benchmark stable** as a *single* rule (was previously documented as two rules pointing opposite ways) |
| Retrieval helps (LA1 / LA2) | tier-stable on HAR (both tiers same direction); HHAR/WISDM tier-stable in opposite direction | EHR fails gradient-headroom; retrieval at-most-tied 5/5 | **Cross-benchmark stable** via the manifold + headroom conjunction |
| Retrieval hurts (LA3 — HHAR/WISDM) | tier-stable at both Full and Tiny; magnitude scales with capacity | not applicable (EHR retrieval not in net-negative regime at n=3) | AdaTime regime split (3/2) |
| `n_cross_layers = 0` for EHR | n/a directly; AdaTime cluster split | n=3 on 5 EHR tasks, all at-most-tied | EHR-side rule |
| Drop fidelity on Mortality-class | n/a (no per-stay binary on AdaTime) | **Yes** (eICU +0.0491 / HiRID +0.0589 both 3/3 p < 0.001) | EHR-only rule |
| Drop MMD on regression | n/a (no continuous regression on AdaTime) | **Yes on KF** (n=3 each cohort); LoS direction stable, magnitude protocol-bound | EHR-only rule |
| HAR-Tiny `(d_ff = 144, dropout = 0.10)` | HAR-tier-specific (n=3) | n/a | HAR-only |
| MFD/SSC saturate at ~68K params (XT) | tier-monotone on MFD, capacity-saturated on SSC | n/a | AdaTime LA2-only |
