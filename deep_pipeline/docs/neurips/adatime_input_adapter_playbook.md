...
- **A3 — `output_mode = "residual"` wins or ties at every measured AdaTime cell** (n=3 multi-seed at all 3 strict-toggle pairs as of the Apr 27 run; jobs 74213–74226 + the s0 baselines, `docs/neurips/playbook_drafts/adatime_multiseed_analysis.md`). Of the 3 strictly toggled RES-vs-ABS pairs at n=3: **2/3 confirm RES at paired Wilcoxon p < 0.001 with Cohen's d ≥ +0.77** (HAR `cap_T_p10`: RES − ABS = **+30.50 MF1** mean over 30 paired (seed × scenario) comparisons, σ 23.29, d = +1.31, Wilcoxon W = 7, p < 1e-5, 27/30 pairs favour RES; WISDM `v4_lr67_fid05` at `p = 10, λ_fid = 0.5`: RES − ABS = **+17.47 MF1** mean over 30 pairs, σ 22.76, d = +0.77, Wilcoxon p = 4e-4, 24/30 pairs favour RES); the third strict toggle (HHAR `v4_base` `p = 10, λ_fid = 0.5`) collapses to a within-σ **statistical tie centred at −0.30 MF1** (σ 11.15 across 30 pairs, d = −0.03, Wilcoxon p = 0.40; per-seed Δ(RES − ABS) = −3.54 / +0.87 / +1.77 — sign-split; mean inside per-seed σ 2.84). The cross-benchmark split co-varies with the predictor + feature regime + `λ_fidelity` coefficient; isolating which axis carries the rule is open (no controlled backbone-swap exists). The previously-load-bearing single-seed claim "HHAR `v4_base_abs` s0 +3.6 MF1" is now **fully retracted at n=3**: it reads as a within-σ tie (per-seed mean = −0.30, σ 2.84), not an ABS-win. **HHAR `v4_base` is the explicit boundary case** the playbook needs to acknowledge — at the awkward `p = 10 × λ_fid = 0.5` corner on a heterogeneous-source dataset, RES and ABS are interchangeable at n=3; pick whichever is operationally simpler. The previously-documented `p > 0 → absolute` direction is refuted: HAR `cap_T_p10` (RES wins +30.50 MF1 at n=3) and WISDM `v4_lr67_fid05` at `p = 10, λ_fid = 0.5` (RES wins +17.47 MF1 at n=3) both have `p > 0` and λ_fid as high as 0.5, and both confirm RES. **Deprecated hypothesis (preserved for evidence-history)**: prior versions of A3 read "use residual IF `pretrain_epochs = 0`; otherwise absolute (provided `λ_recon > 0`)". That direction was supported by exactly one single-seed AdaTime cell (HHAR `v4_base_abs` s0, +3.6 MF1) which is now a within-σ tie at n=3. The cross-benchmark split is *not* keyed on `pretrain_epochs`. The replacement framing — "predictor + feature regime" — names a *combination* of co-varying axes (predictor architecture, feature dimensionality + type, `λ_fidelity` coefficient regime); these axes have not been separately isolated by a controlled backbone-swap. A 1D-CNN-on-EHR or 4-layer-LSTM-on-AdaTime pilot is the highest-leverage missing experiment for separating predictor from features (open follow-up §9 #7). The magnitude modulator survives directionally: across the four `p = 0` AdaTime datasets, Pearson `r(λ_fidelity, residual advantage)` ≈ **−0.93 (n = 4, two-tailed p ≈ 0.07; Spearman ρ ≈ −0.85)** (`output_mode_multivariable_audit.md` Phase 3) — directional pattern is consistent with an inverse relationship; n is too small for conventional significance. λ_fidelity appears to set the *size* of the residual win (HAR 0.10, SSC/WISDM 0.01, HHAR 0.5, MFD 1.0; new data adds `λ_fid = 0.5` at HHAR-p10 and WISDM-p10), and residual wins at every λ_fidelity tested. On EHR, R7 forces `p > 0` and `lambda_recon > 0`, and absolute wins on 5/5 tasks at n=3 (clean C8 strict toggle vs C0 `mort_c2`, `aki_v5_cross3`, `sepsis_v5_cross3`, `los_v5_cross3`, `kf_v5_cross3`); the EHR per-task verdict is unchanged. The boundary case `aki_nf_C8` (residual + `lambda_recon = 0` at `p > 0`) lands at ≈ +0.0002 single-seed; this is now reread as "residual is the universal AdaTime-style default *and* survives marginally when EHR's fidelity anchor is also removed", not as a special boundary of the deprecated `p`-keyed rule. **KF severity note** (unchanged): residual on KF is additionally catastrophic because `cum_max`/`cum_min` recompute integrates δ-drift forward — but EHR's predictor + feature regime already routes KF to absolute. Mechanism (§6) is rewritten accordingly.
...
| `output_mode` AdaTime → residual; EHR → absolute | wins-or-ties on 5/5 AdaTime at n=3 (3 strict-toggle pairs: 2 RES-wins-at-Wilcoxon-p<0.001, 1 within-σ tie at HHAR `v4_base`, p = 0.40) | **Yes 5/5** EHR (clean C8 strict toggle, n=3) | Cross-benchmark pattern descriptive; mechanism axis (predictor / feature-dim / λ_fid) co-varying and not isolated | **STRONG-within-evidence (AdaTime)**, STRONG (EHR descriptive); WEAK on mechanism attribution |
...
9. **HHAR-tie generalisation.** Does the HHAR `v4_base` within-σ RES/ABS tie (Wilcoxon p = 0.40 at n=3) persist under different `λ_fid` (e.g. 0.1, 1.0) or different adapter capacity tiers? Equivalently — is the tie a property of (i) the heterogeneous-source dataset family, (ii) the high-`λ_fid` (≥0.5) corner, (iii) `p > 0`, or some interaction? Required: 6–9 jobs (HHAR `v4_base` × {`λ_fid ∈ {0.1, 1.0}`, Tiny vs Full} × n=3) to disambiguate.
...
- **A3 has been rewritten (Apr 26) and hardened to STRONG-within-evidence at n=3 (Apr 27): `p > 0 → absolute` is refuted on AdaTime; AdaTime is universal-residual with one boundary-case tie.** The Apr 27 multi-seed run resolves the single-seed caveat: all 3 strict-toggle pairs (HAR `cap_T_p10`, WISDM `v4_lr67_fid05`, HHAR `v4_base`) are now at n=3 (`adatime_multiseed_analysis.md`). HAR's RES-vs-ABS gap *strengthens* at multi-seed (+24.86 MF1 single-seed → **+30.50 MF1** mean over 30 paired (seed × scenario) comparisons, paired Wilcoxon p < 1e-5, Cohen's d = +1.31); WISDM `v4_lr67_fid05` is reproduced and slightly amplified (+13.41 → **+17.47 MF1** at multi-seed, p = 4e-4, d = +0.77, σ on RES MF1 = 0.19 — practically deterministic). The HHAR `v4_base` cell collapses to a within-σ **statistical tie** at n=3 (per-seed Δ = −3.54 / +0.87 / +1.77, mean −0.30 ± σ 2.84, Wilcoxon p = 0.40); the previously-load-bearing single-seed claim "HHAR `v4_base_abs` s0 +3.6 MF1" is fully retracted. The remaining caveat is **dataset coverage**: 3 strict-toggle pairs across 3 AdaTime datasets at n=3 is below community-standard (5 datasets × 5 seeds). The HHAR `v4_base` tie is the new explicit boundary case; future work should test whether the tie generalises to other (`p > 0, λ_fid ≥ 0.5`) configurations on heterogeneous-source datasets. The rule is "RES wins or ties at every measured AdaTime cell at n=3 across 3 datasets" — not ">3σ everywhere on community-standard 5×5".
