...
**One preliminary regime-split hypothesis** (the central novel finding; §3 below) — **directional only**, two-point calibration:

- **R4 — Whether to drop fidelity tracks task-gradient label-density at the relevant granularity, with `cos(task, fidelity)` as a candidate mechanistic decomposition.** *Directly measured on 2 of 5 tasks*: Sepsis (cos = −0.21, 1.13% per-timestep positive rate) collapses by −0.0690 ± 0.0167 AUROC under `lambda_recon = 0` (`sepsis_playbook_analysis.md` B.3 — the recon term anchors the encoder against an orthogonal task gradient; BCE saturates to NaN within 8 epochs). Mortality (cos = +0.84, dense per-stay gradient) goes the other way. LoS, KF, and AKI cosines are *not directly measured* — their C5 verdicts are inferred from density alone. KF additionally shows seed-level instability at default lr=1e-4 because cumulative-feature recomputation integrates drift through `cum_max`. The operational protocol is to fork a 1–3 epoch pilot and read the gradient cosine off the existing `[grad-diag]` log line; the provisional threshold of cos ≥ +0.5 with positive-timestep fraction ≥ 5% is **a midpoint between the two measured points (Mortality +0.84, Sepsis −0.21)**, not a calibrated boundary. Calibration on the three unmeasured EHR tasks is the highest-priority follow-up (§7 #2). Until then, the density-only rule is the safer operational form.
...
4. Decision (provisional, two-point calibration only):
   - If cos > 0 (cooperative) and labels are dense at the relevant granularity (per-batch positive fraction ≥ 5%, or task is dense regression): consider setting `lambda_recon = 0`; expect ~+0.005 AUROC / −0.005 MAE on tasks with the Mortality signature.
   - If cos < 0 or positive-timestep fraction < 5% per batch: keep `lambda_recon` at default; do not drop.
   - Borderline (cos ∈ [0, +0.5]): keep default — n=3 evidence is missing in this band.

The +0.5 cosine threshold is **provisional and not calibrated** — it is a midpoint between the only two directly-measured cosines (Mortality +0.84 → win; Sepsis −0.21 → loss). Any threshold in the open interval (−0.21, +0.84) would split those two points equally well, including the literature-canonical sign rule (cos > 0; PCGrad — Yu et al. 2020, `arXiv:2001.06782`) and a density-only rule. LoS, KF, AKI are inferred from the dense-vs-sparse axis without direct cos measurement. A 5-task pilot measuring cos directly on LoS / KF / AKI is the highest-priority follow-up (§7 #2).
...
**`lambda_recon` (autoencoder reconstruction / "fidelity" anchor in retrieval trainer).** *Rule (preliminary)*: drop when cos(task, fidelity) > 0 (cooperative) and labels are dense at the relevant granularity. *Evidence*: cohort-stable on Mortality (n=3 both cohorts); near-neutral on LoS; seed-fragile on KF at default lr; catastrophic on Sepsis. *Mechanism*: when redundant with the task signal, fidelity over-regularises through a cooperative ratio of ≈ 2.81× on Mortality (37% of task gradient adds no direction); when orthogonal and labels are sparse (cos = −0.21 on Sepsis, fid 3–10× task at f=20 oversample), fidelity is the term that empirically anchors the encoder in the source-input distribution (observed at n=3 on Sepsis cross3). The 3–10× ratio is Sepsis-specific, not EHR-wide. *Scope*: rule direction is measurable on the first epoch via `[grad-diag]` cosine; +0.5 threshold is unfitted. *Decision-tree leaves*: L1, L3 drop; L2/L4/L5 keep.
...
| R4 — cos × density rule (preliminary) | Mortality (+0.84 → win) and Sepsis (−0.21 → loss) directly measured; LoS / KF / AKI cosines inferred | Mortality replicates; Sepsis pending | **DIRECTIONAL — two-point calibration; threshold +0.5 unfitted; LoS/KF/AKI cos pending** |
