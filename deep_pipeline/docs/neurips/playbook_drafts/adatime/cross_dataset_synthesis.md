...
* **I-A3 — Output-mode rule rewritten (Apr 26) and hardened to STRONG-within-evidence (Apr 27): residual wins-or-ties on every measured AdaTime cell at n=3; absolute wins on 5/5 EHR tasks at n=3. Cross-benchmark split co-varies with three axes — predictor architecture, feature dimensionality + type, and `λ_fidelity` regime — that have not been separately isolated.** AdaTime: at n=3 across 3 strict-toggle pairs (`adatime_multiseed_analysis.md` Phases 2–3), 2/3 confirm RES at paired Wilcoxon p < 0.001 with Cohen's d ≥ +0.77 (HAR `cap_T_p10` RES − ABS = **+30.50 MF1** mean over 30 paired (seed × scenario) comparisons, p < 1e-5, d = +1.31; WISDM `v4_lr67_fid05` `p=10, λ_fid=0.5` RES − ABS = **+17.47 MF1**, p = 4e-4, d = +0.77); the third (HHAR `v4_base` `p=10, λ_fid=0.5`) collapses to a within-σ statistical tie centred at **−0.30 MF1** (Wilcoxon p = 0.40, per-seed Δ = −3.54 / +0.87 / +1.77). Both directionally-discriminating cells *strengthen* at multi-seed (HAR +24.86 → +30.50; WISDM `v4_lr67` +13.41 → +17.47); HAR's previously-load-bearing single-seed claim "HHAR `v4_base_abs` s0 +3.6 MF1" is fully retracted as a within-σ tie at n=3. EHR: absolute on 5/5 at n=3 via C8 strict toggle. The "predictor-architecture-keyed" reading is one of three co-varying explanations and is not separately tested by a controlled backbone-swap. The deprecated `p`-keyed bridge is preserved in `output_mode_multivariable_audit.md` Phase 6 as evidence-history. **HHAR `v4_base` p=10 is the explicit boundary case** the rule must acknowledge — at the heterogeneous-source × `p > 0` × `λ_fid = 0.5` corner, RES and ABS are interchangeable at n=3.
...
**AdaTime rule (rewritten Apr 26; n=3 confirmed Apr 27).** **Residual is universal on AdaTime — it wins or ties at every measured `pretrain_epochs × λ_fidelity` cell across HAR, HHAR, WISDM, SSC, MFD.** The Apr 27 multi-seed run (`adatime_multiseed_analysis.md` Phases 2–3; jobs 74213–74226 + s0 baselines) confirms residual at n=3 on the discriminating cells, and refutes the previously-documented `p > 0 → absolute` direction:
...
| HHAR `v4_base` `p=10, λ_fid=0.5` | 87.80 ± 1.92 | 88.10 ± 1.26 | **−0.30** (per-seed Δ = −3.54 / +0.87 / +1.77, σ 2.84) | 0.40 | −0.03 | 3 | 0.50 | 10 |
| HHAR `cap_T_p0` ABS-only (RES = single-seed baseline) | (existing) ≈91.73 | 80.27 ± 3.35 | RES > ABS by ~+11.5 (mixed-n) | — | — | 3 (ABS) / 1 (RES) | 0.50 | 0 |
| WISDM `cap_T_p0` ABS-only (RES = single-seed baseline) | (existing) ≈80.38 | 48.63 ± 3.08 | RES > ABS by ~+31.8 (mixed-n) | — | — | 3 (ABS) / 1 (RES) | 0.01 | 0 |

The single AdaTime cell that previously supported `p > 0 → absolute` (HHAR `v4_base` s0, +3.6 MF1) is fully retracted at n=3 as a within-σ tie (Wilcoxon p = 0.40, d = −0.03). The `p > 0` direction also fails on HAR (RES wins by +30.50 at n=3) and on WISDM at the `λ_fid = 0.5` cell (RES wins by +17.47 at n=3) — neither pretrain nor a higher fidelity coefficient is sufficient to flip AdaTime to absolute.
...
* **AdaTime regime.** Frozen 1D-CNN + raw low-dim time-series → residual universal (3/3 strict-toggle pairs at n=3 RES wins or ties; 2 RES wins at Wilcoxon p < 0.001, 1 within-σ tie at HHAR `v4_base`).
...
**Deprecated rule (preserved as evidence-history).** Prior versions of this section read: "use residual IF `pretrain_epochs = 0`; otherwise absolute (provided `λ_recon > 0`)". That direction was supported by exactly one single-seed AdaTime cell (HHAR `v4_base_abs` s0, +3.6 MF1) which is now a within-σ tie at n=3 (Wilcoxon p = 0.40). The full audit is in `output_mode_multivariable_audit.md` Phase 6.
...
* **Confidence.** EHR side: STRONG (5/5 tasks at n=3, clean C8 strict toggle). AdaTime side: **STRONG-within-evidence** at n=3 across 3 strict-toggle pairs (`adatime_multiseed_analysis.md` Phases 2–3): HAR `cap_T_p10` RES − ABS = +30.50 MF1 (Wilcoxon p < 1e-5, d = +1.31); WISDM `v4_lr67_fid05` p=10 RES − ABS = +17.47 MF1 (p = 4e-4, d = +0.77); HHAR `v4_base` p=10 is a within-σ tie (mean −0.30 MF1, p = 0.40). The remaining caveat is dataset coverage (3 datasets × 3 seeds is below community-standard 5×5). The previous "regime-split by `pretrain_epochs`" framing is **deprecated** — see `output_mode_multivariable_audit.md` Phase 6. Mechanism attribution: WEAK pending the §10 #4 backbone-swap experiment.
...
| `output_mode` AdaTime → residual; EHR → absolute | **Yes** (5/5 EHR pick absolute via C8 strict toggle at n=3) | wins-or-ties on 5/5 AdaTime at n=3 (3 strict-toggle pairs: 2 RES-wins-at-Wilcoxon-p<0.001, 1 within-σ tie at HHAR `v4_base`, p = 0.40) | Cross-benchmark pattern descriptive; mechanism axis (predictor / feature-dim / λ_fid) co-varying and not isolated; deprecated `p`-keyed bridge in `output_mode_multivariable_audit.md` Phase 6 | **STRONG-within-evidence (AdaTime)**, STRONG (EHR descriptive); WEAK on mechanism attribution |
...
4. **`output_mode`: AdaTime wins-or-ties on residual at every measured cell; EHR universal-absolute.** AdaTime: 3 strict-toggle pairs at n=3 (`adatime_multiseed_analysis.md` Phases 2–3) — HAR `cap_T_p10` RES − ABS = +30.50 MF1 (Wilcoxon p < 1e-5, Cohen's d = +1.31); WISDM `v4_lr67_fid05` `p=10, λ_fid=0.5` RES − ABS = +17.47 MF1 (p = 4e-4, d = +0.77); HHAR `v4_base` `p=10, λ_fid=0.5` is a within-σ tie (mean −0.30 MF1, p = 0.40, d = −0.03). EHR: 5/5 tasks pick absolute via C8 strict toggle at n=3. The cross-benchmark mechanism co-varies with three axes (predictor architecture, feature dimensionality + type, λ_fidelity coefficient regime); the predictor-architecture-keyed reading is one of three explanations and not separately isolated by a controlled backbone-swap (open follow-up §10 #4). λ_fidelity appears to set the *magnitude* of the residual win on AdaTime (Pearson r ≈ −0.93 across `p = 0` datasets; **n = 4, two-tailed p ≈ 0.07; Spearman ρ ≈ −0.85** — directional only). Previously-documented `p`-keyed bridge is **deprecated**; see `output_mode_multivariable_audit.md` Phase 6. **Honest seed-count flag**: AdaTime evidence is now n=3 across 3 datasets (below community-standard 5×5 but above the previous single-seed coverage); HHAR `v4_base` is the explicit within-σ tie boundary case.
