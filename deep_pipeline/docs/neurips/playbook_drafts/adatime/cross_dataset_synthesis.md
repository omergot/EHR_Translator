...
* **I-A3 — Output-mode rule rewritten (Apr 26): residual is universal on AdaTime; absolute is universal on EHR. The cross-benchmark split is keyed on predictor + feature regime, not on `pretrain_epochs`.** AdaTime: 7 strict-toggle pairs from the Apr 26 claim-strengthening run (`adatime_claim_strengthening_run.md` Phase 4) all show residual winning or tying — including HAR `cap_T_p10` (RES +24.86 MF1) and WISDM `v4_lr67_fid05` at p=10 (RES +13.41 MF1) which directly refute the previously-documented `p > 0 → absolute` direction. The pivotal HHAR `v4_base` cell, which previously supported `p > 0 → absolute` at single-seed (+3.6 MF1, s0), now reverses to RES +0.87 MF1 at s1; 2-seed mean is ABS +1.36 MF1 within σ ≈ 1.4 — i.e. a tie, not a sign flip. EHR: absolute on 5/5 at n=3 via C8 strict toggle, predictor-architecture-keyed (frozen LSTM + tabular features). The deprecated `p`-keyed bridge is preserved in `output_mode_multivariable_audit.md` Phase 6 as evidence-history.
...
The 5 AdaTime datasets fall cleanly into two clusters by manifold-coherence proxy:

* **Retrieval-positive cluster** — **HAR / SSC / MFD.** Medium-to-high target-manifold coherence: HAR's UCI-HAR was acquired on a single Samsung Galaxy SII smartphone with identical waist placement on every subject (`har_playbook_analysis.md` §D.2; Anguita et al. 2013); SSC is wide-sense-stationary 30-second EEG epochs (rhythm-signature-discriminative, single-montage clinical recording); MFD is 1-channel × 5120-timestep stationary rotating-machinery vibration with characteristic bearing-fault frequencies (BPFI/BPFO/BSF, Lessmeier et al. 2016). For each, the encoder representation space is low-dimensional and smooth; k-NN cross-attention queries return *informative* neighbours that share the discriminative signature with the query.
* **Retrieval-negative cluster** — **HHAR / WISDM.** Low manifold coherence: HHAR spans 9 distinct devices × 9 subjects (Stisen et al. 2015 "Smart Devices are Different"); WISDM spans 36 subjects × uncontrolled smartphone-in-pocket placement (Kwapisz et al. 2010). For both, k-NN returns neighbours from a *different device or subject cluster than the query*, and cross-attention mixes wrong-cluster features into the representation. The penalty scales with cluster heterogeneity: −0.026 (HHAR-Full, 9 clusters) → −0.074 (WISDM-Full, 36 clusters).
...
The unified rule is: **default `p > 0` for EHR-style frozen-recurrent + composite-loss + sparse-or-conflicting-gradient setups; default `p = 0` for AdaTime-style frozen-CNN + dense-balanced-softmax + low-dim-feature setups**. The gradient-cosine measurement at first epoch (already wired into the EHR `[grad-diag]` log per `gradient_bottleneck_analysis.md`) is the operational discriminator: if cos(task, fidelity) ≥ +0.5 *and* fid/task ratio < 1 *and* signal channel count < 5, default `p = 0`; otherwise default `p ≥ 10`.
...
**Mechanistic synthesis (rewritten).** The cross-benchmark split is keyed on **predictor + feature regime**, not on `pretrain_epochs` or `λ_fidelity`. Frozen 1D-CNN over raw low-dim time-series (AdaTime) → residual; frozen LSTM over tabular ICU features (EHR) → absolute. Residual produces output ≈ `input + small_noise` at random init, giving an implicit output-magnitude floor that AdaTime's small-coefficient fidelity regime cannot match without exact `λ_fid` tuning. Pretrain warms an absolute decoder, but on AdaTime the warm decoder is still beaten by residual's implicit floor (HAR `cap_T_p10`, WISDM `v4_lr67_fid05` p=10). On EHR, the frozen LSTM + tabular features regime selects absolute regardless of `p` or `λ_fid` (5/5 C8 strict toggles).
...
* **Operational test.** Measure `cos(∇task, ∇fidelity)` and `‖∇fid‖ / ‖∇task‖` at Phase-2 epoch 1. If cos ≥ +0.5 and fid/task < 1 and channel count < 5 → default `p = 0`. Otherwise → default `p ≥ 10`.
* **Confidence.** High in direction (5/5 each side); the operational threshold is calibrated only on Mortality (cos = +0.84) and Sepsis (cos = −0.21). Other thresholds are inferred.
...
### Rank 6 — Output mode predictor-architecture-keyed: AdaTime universal-residual; EHR universal-absolute

* **Mechanism.** §5 (rewritten Apr 26). Cross-benchmark split is keyed on the predictor + feature regime, not on `pretrain_epochs`. Frozen 1D-CNN over raw low-dim time-series (AdaTime) → residual gives an implicit output-magnitude floor that explicit fidelity anchors cannot match without exact `λ_fid` tuning. Frozen LSTM over tabular ICU features (EHR) → absolute, because tabular features destabilise under residual `δ` perturbations and KF additionally has cumulative-stat recompute.
* **Confidence.** EHR side: high (5/5 tasks at n=3, clean C8 strict toggle). AdaTime side: 7/7 strict-toggle pairs from the Apr 26 claim-strengthening run show RES wins or ties, including HAR `cap_T_p10` RES +24.86 MF1 and WISDM `v4_lr67_fid05` p=10 RES +13.41 MF1; HHAR `v4_base` p=10 is now a 2-seed within-σ tie. 6 of the 7 new pairs are single-seed (caveat); HHAR `v4_base` p=10 is n=2. The previous "regime-split by `pretrain_epochs`" framing is **deprecated** — see `output_mode_multivariable_audit.md` Phase 6.
...
| Stability claim | EHR cohort-stable? (eICU↔HiRID at n=3) | AdaTime tier-stable? (Full↔Tiny at n=3) | Cross-benchmark? |
|---|---|---|---|
| Drop fidelity on Mortality-class | **Yes** (+0.0491 / +0.0589) | n/a | EHR-only rule |
| Drop MMD on regression | **Yes on KF** (n=3 each cohort); LoS direction stable, magnitudes protocol-bound | n/a | EHR-only rule |
| `output_mode` predictor-architecture-keyed (AdaTime → residual; EHR → absolute) | **Yes** (5/5 EHR pick absolute via C8 strict toggle at n=3) | **Yes** (7/7 strict-toggle pairs from Apr 26 run pick residual; only previously-supporting `p > 0 → absolute` cell, HHAR `v4_base` s0, collapses to within-σ tie at s1) | **Yes — single predictor-keyed rule §5; deprecated `p`-keyed bridge in `output_mode_multivariable_audit.md` Phase 6** |
| `n_cross_layers = 0` for EHR | n=3 on 5 EHR tasks, all at-most-tied; HiRID Sepsis just landed at n=3 picking C1 (+0.0689 ± 0.0113) | n/a directly; cross-benchmark cluster split | **Yes on AdaTime cluster split (R-A1)** |
| Tiny ≥ Full | not directly tested (Goal-A/B is different design) | **Yes 5/5** (HAR/HHAR/WISDM/SSC/MFD) | **AdaTime tier-stable**; EHR open |
| `p = 0` universal | EHR catastrophic (5/5 hurts); pretrain mandatory | **Yes 5/5** | **Cross-benchmark regime split** (clean) |
| Retrieval meta-rule (manifold + headroom) | EHR fails (ii); retrieval at-most-tied | AdaTime cluster splits 3/2 on (i); both clusters trivially satisfy (ii) | **Cross-benchmark stable** (the conjunction is the unified rule) |
| HiRID Sepsis catastrophe (C5) | **pending**: HiRID Sepsis n=3 used C1 not C5; the C5 catastrophe cohort-stability is still open | n/a | EHR-only follow-up |
...
* **Cai, R.; Chen, J.; Li, Z.; Chen, W.; Zhang, K.; Ye, J.; Li, Z.; Yang, X.; Zhang, Z.** "Time Series Domain Adaptation via Sparse Associative Structure Alignment (CoTMix)." AAAI 2021. `arXiv:2012.11797`. AdaTime baseline; confirms heterogeneous time-series DA needs explicit mechanism for cross-cluster alignment, which retrieval at-training does not provide on HHAR/WISDM.
...
4. **`output_mode` predictor-architecture-keyed: residual universal on AdaTime; absolute universal on EHR.** AdaTime: 7/7 strict-toggle pairs from the Apr 26 claim-strengthening run show RES wins or ties (HAR `cap_T_p10` RES +24.86 MF1; WISDM `v4_lr67_fid05` p=10 RES +13.41 MF1; HHAR `v4_base` p=10 is now a 2-seed within-σ tie). EHR: 5/5 tasks pick absolute via C8 strict toggle at n=3. The cross-benchmark mechanism is the predictor + feature regime (frozen 1D-CNN + raw low-dim → residual; frozen LSTM + tabular → absolute), *not* `pretrain_epochs`. λ_fidelity sets the *magnitude* of the residual win on AdaTime (Pearson r ≈ −0.93 across `p = 0` datasets), not its direction. Previously-documented `p`-keyed bridge is **deprecated**; see `output_mode_multivariable_audit.md` Phase 6. **Honest seed-count flag**: 6 of the 7 new AdaTime pairs are n=1.
5. **Drop fidelity (`λ_recon = 0`) only when cos(task, fidelity) ≥ +0.5 AND task gradient is dense at the relevant granularity.** EHR-specific operational rule (`input_adapter_playbook.md` §3). Mortality (cos=+0.84) → drop, family-best on both eICU and HiRID at n=3. Sepsis (cos=−0.21) → keep, catastrophic if dropped. Cross-benchmark relevance: AdaTime with `lambda_fidelity = 0.01–0.5` is in the small-coefficient regime where the rule trivially predicts "keep fidelity" — consistent with AdaTime practice. **Cohort-stable on EHR; predictively consistent with AdaTime practice.**
